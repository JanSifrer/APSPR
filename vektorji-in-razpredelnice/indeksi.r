...
# 
#     y <- c(3, 7, 1, 5, 9)
#     order(y) <- za urejanje elementov indeksa
#     y[order(y)] <- dobiš urejene idenkse
#     
...
obrestuj <- function(stanja) {
  ...
  stanja[stanja < 0] <- stanja[stanja < 0] * 1.1
  stanja
...
check$summarize <- function() {
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjAyLCJwYXJ0IjoxNzk3MX0:1gFZL6:yZLyqL5V6ALGnDWbGItEAYS4mXM'
    tryCatch({
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjAyLCJwYXJ0IjoxNzk3Mn0:1gFZL6:Sag0hLy6DQgi0CK8iXP0FcH5jrQ'
    tryCatch({
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjAyLCJwYXJ0IjoxNzk3M30:1gFZL6:F8sisVAFRKBAjQkltHAlahUVy_Q'
    tryCatch({
